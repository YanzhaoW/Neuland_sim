...
Bool_t R3BNeuland::ProcessHits(FairVolume*)
{
    ...
    // auto stack_temp = gMC->GetStack();
    // auto particle_temp = stack_temp ->GetCurrentTrack();
    // gMC->TrackPosition(fPosOut);
    // gMC->CurrentVolOffID(1, fPaddleID);
    // std::cout << "------------" << std::endl;
    // std::cout << "CurrentTN: "<< stack_temp->GetCurrentTrackNumber() << "\tNtrack: " << stack_temp->GetNtrack()  << 
    //     "\tparticle: " <<particle_temp->GetName()<< "\t(" << fPosIn.X() << ", " << fPosIn.Y() << ", " << fPosIn.Z()<< ")\t paddleID: "<< fPaddleID<< std::endl;
